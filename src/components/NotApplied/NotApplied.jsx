...
import Archived from "../Archive/Archived";
...
function NotApplied() {
  ...
  const fetchStudentData = async (params = {}) => {
    ...
    try {
      ...
      // Prepare POST payload - Include email as the search parameter
      const payload = {
        page: apiPage,
      ...
      const response = await api.post(
        `${BASE_URL}/profile/studentIntake2`,
        payload,
        {
          headers: {
            Authorization: `Bearer ${accessToken}`,
  ...
  // Handle going back to NotApplied from Archived
  ...
  if (showDeleted) {
    return <Deleted onBack={handleBackToNotApplied} />;
  }

  // Show Archived tab if showArchived is true
  if (showArchived) {
    return <Archived onBack={handleBackFromArchived} />;
  }
