...
import { fetchInternshipRegisterStatus } from "../../../store/Slices/internship/internRegisterStatus";

const Widgets = ({ refresh, setRefresh }) => {
  const theme = useTheme();
  const colors = tokens(theme.palette.mode);
  const dispatch = useDispatch();
  const internshipStatus = useSelector(
    (state) => state.internshipStatusRegisterSlice.data
  );
  const isLoading = useSelector((state) => state.internshipStatus.isLoading);

  useEffect(() => {
    dispatch(fetchInternshipRegisterStatus());
  }, [refresh]);
  ...
  const internship = internshipStatus.map((item) => {
    ...
    } else if (item.title === "Approval Pending") {
