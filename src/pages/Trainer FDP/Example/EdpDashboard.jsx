...
const EdpDashboard = () => {
  ...
  const fetchFilters = async () => {
    try {
      const res = await api.get(`/event/filter/domains-institutes?event_type=edp`);
  ...
  return (
    <div className="p-4 md:p-6 relative">
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-8">
          ...
          <div className="grid gap-4 md:gap-6 grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-4 xl:grid-cols-4 items-start">
            {[
              ...
              { label: "Educators Trained", value: metrics.trained, icon: <Users className="text-indigo-600" />, color: "bg-indigo-50" },
              { label: "Educators Failed", value: metrics.failed, icon: <Users className="text-red-600" />, color: "bg-red-50" },
              { label: "Educators Dropouts", value: metrics.dropout, icon: <Users className="text-gray-600" />, color: "bg-gray-50" },              
      ...
      {/* Bookings Tab */}
      {activeTab === "bookings" && <StaticBookedDetails eventTypeFilter="edp" />}
    </div>
  );
};
